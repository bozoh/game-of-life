 ...
 *
 * Universe.java A universe is a succession of grids over time. Each new grid is
 * generated from the previous one using the rules of the Game Of Life.
 */
public class Universe {
	...
	// Calculates the game-of-life's next iteration, called on "Next Generation"
	// button click
	...
	public void createNextGeneration() {
		...
		for (int y = 0; y < maxRow; y++) { // From top to bottom
			for (int x = 0; x < maxColumn; x++) { // From left to right
				...
				int neighbourCount = currentGridContent.getLiveNeighboursAt(x, y); // getLiveNeighboursAt()
																					// defined
																					// in
																					// Grid.java
				Cell nextCell = getNextCellState(currentCell, neighbourCount);
	...
	private Cell getNextCellState(Cell currentCell, int neighbourCount) {
		// Based on the rules of game-of-life, calculate next state
		if (currentCell == Cell.LIVE_CELL) { // If cell is currently LIVE
			if ((neighbourCount == 2) || (neighbourCount == 3)) {
				return LIVE_CELL; // Stay LIVE if 2 or 3 neighbours are LIVE
			} else {
				return DEAD_CELL; // Else, become DEAD due to underpopulation or
									// overcrowding
			}
		} else { // If cell is currently DEAD
			if (neighbourCount == 3) {
				return LIVE_CELL; // Become LIVE if 3 neighbours are LIVE
			} else {
				return DEAD_CELL; // Else, stay DEAD
			}
		}
	}
